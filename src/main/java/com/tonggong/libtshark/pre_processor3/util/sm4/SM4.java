...
public class SM4 {
    ...
    private byte[] padding(byte[] input, int mode) {
        ...
        if (mode == SM4_ENCRYPT) {
            ...
            String inputHex = ByteUtils.byteToHex(input) + "00";
            StringBuilder stringBuffer = new StringBuilder(inputHex);
            for (int i = 0; i < p - 1; i++) {
                stringBuffer.append("00");
            }
            ret = ByteUtils.hexToByte(stringBuffer.toString());
...
//            ret= Util.hexToByte(substring);
            ...
            ret = ByteUtils.division(input, 0, index + 1);
        }
        return ret;
    }
